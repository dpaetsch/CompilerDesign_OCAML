...
let rec print_stream_helper (s:stream) : unit = 
  match s with
  | [] -> ()
  | (L l)::t -> Printf.printf "L %s\n" l; print_stream_helper t
  | (I (u, i))::t -> Printf.printf "I %s: %s\n" u (Llutil.string_of_insn i); print_stream_helper t
  | (T t)::ts -> Printf.printf "T %s\n" (Llutil.string_of_terminator t); print_stream_helper ts
  | (G (g, gd))::ts -> Printf.printf "G %s: %s\n" g (Llutil.string_of_gdecl gd); print_stream_helper ts
  | (E (u, i))::ts -> Printf.printf "E %s: %s\n" u (Llutil.string_of_insn i); print_stream_helper ts
...
(* ADDED: Returns nothing, filler to prevent labels form disappearing*)
let return_nothing (rt: Ll.ty) : stream = 
  begin match rt with
    | Void -> [T (Ret (Void, None))]
    | I1 -> [T (Ret (I1, Some (Const 0L)))]
    | I8 -> [T (Ret (I8, Some (Const 0L)))]
    | I64 -> [T (Ret (I64, Some (Const 0L)))]
    | Ptr t -> [T (Ret (Ptr t, Some(Null)))]
    | Struct t -> [T (Ret (Struct t, Some(Null)))]
    | Array (num, ty) -> [T (Ret (Array (num, ty), Some(Null)))]
    | Fun (argty, retty) -> [T (Ret (Fun (argty, retty), Some(Null)))]
    | Namedt t -> [T (Ret (Namedt t, Some(Null)))]
  end
  (* *)
  ...
  *)
  ...
  | Ast.CStruct (id, l) ->
    ...
    let store_exp_code, cmp_exp_code = List.fold_left (fun (f_code, stream) (f_id, f_exp) -> 
      ...
      let field_ptr_id, field_value_id = gensym "field_ptr_id", gensym "field_value_id" in
...
and cmp_stmt (tc : TypeCtxt.t) (c:Ctxt.t) (rt:Ll.ty) (stmt:Ast.stmt node) : Ctxt.t * stream =
  ...
  | Ast.Assn (path ,e) ->
    ...
    let rhs_ty, eop, exp_code = cmp_exp tc c e in
    c, path_code >@ exp_code >:: I("", (Store (rhs_ty, eop, pop)))
  ...
  *)
  | Ast.Cast (typ, id, exp, notnull, null) ->
    ...
    (* result, return ctxt and stream *)
      (* ctxt *)
      c, exp_code >:: I (cmp_id, Icmp (Ne, exp_ty, exp_op, Null)) >:: T (Cbr (Id cmp_id, not_null_id, null_id))
      >:: L not_null_id 
      (* allocate the type *)
      >:: E(id, Alloca exp_ty) 
      (* store with operand *)
...
let cmp_fdecl (tc : TypeCtxt.t) (c:Ctxt.t) (f:Ast.fdecl node) : Ll.fdecl * (Ll.gid * Ll.gdecl) list =
  ...
  let block_code = 
    let code = cmp_block tc c ll_rty body in
    begin match code with 
      | [] -> return_nothing ll_rty
      | last::rest -> 
        begin match last with
          | T _ -> code
          | _ -> code >@ return_nothing ll_rty
      end
    end in
