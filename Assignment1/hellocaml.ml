...
let rec insert (x:'a) (l:'a list) : 'a list =
  begin match l with
    | [] -> [x]
    | y::ys -> if (x = y) then y::ys else (if x < y then x::y::ys else y::insert x ys)
  end
...
let rec lookup (x:string) (c:ctxt) : int64 =
  begin match c with
    | [] -> raise Not_found
    | (k,v)::ys -> if (x = k) then v else lookup x ys
  end
...
let rec optimize (e:exp) : exp =
  begin match e with
    ...
    | Add(e1, e2) -> begin match (optimize e1, optimize e2) with
      ...
      | (Var x, Neg(Var y)) -> if (x = y) then (Const 0L) else Add(optimize e1, optimize e2)
      | (Neg(Var x), Var y) -> if (x = y) then (Const 0L) else Add(optimize e1, optimize e2)
    ...
    | Mult(e1, e2) -> begin match (optimize e1, optimize e2) with
      ...
      | (Neg(x), Neg(y)) -> Mult(optimize x, optimize y)
      | _ -> Mult(optimize e1, optimize e2)
      end
    | Neg(e1) -> begin match optimize e1 with
      | Neg(x) -> optimize x
      | (Add(e2,e3)) -> optimize (Add(Neg(e2),Neg(e3)))
      | Const 0L -> Const 0L
      | _ -> Neg(optimize e1)
      end
  end
