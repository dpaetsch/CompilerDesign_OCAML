...
module Alloc = struct
  ...
  let of_block (f : Ll.uid -> loc) (g : Ll.gid -> X86.lbl)
      (live_in : uid -> UidSet.t) (b : Ll.block) : fbody =
    List.map
      (fun (u, i) ->
  ...
  let of_cfg (f : Ll.uid -> loc) (g : Ll.gid -> X86.lbl)
      (live_in : uid -> UidSet.t) ((e, bs) : Ll.cfg) : fbody =
    List.(
...
let print_layout (layout : layout) : string =
  let print_loc uid = Printf.sprintf "%s -> %s" uid (Alloc.str_loc (layout.uid_loc uid)) in
  let locs = List.map print_loc (UidSet.elements (UidSet.empty)) in
  String.concat "\n" locs
...
let caller_save : LocSet.t =
  [ Rax; Rdi; Rsi; Rdx; Rcx; R08; R09; R10; R11 ]
...
let arg_loc (n : int) : Alloc.loc =
  match arg_reg n with Some r -> Alloc.LReg r | None -> Alloc.LStk (n - 4)
...
let compile_pmov live (ol : (Alloc.loc * Ll.ty * Alloc.operand) list) :
  ...
  let rec loop outstream ol =
    ...
    | (x, _, o) :: ol', [] ->
        let os =
          loop (outstream >:: I Asm.(Pushq, [ compile_operand o ])) ol'
        in
...
let compile_call live (fo : Alloc.operand) (os : (ty * Alloc.operand) list) :
  ...
  >@ lift
       (if nstack <= 0 then [] else Asm.[ (Addq, [ ~$(nstack * 8); ~%Rsp ]) ])
  ...
  let rec loop (af : Alloc.fbody) (outstream : x86stream) : x86stream =
    ...
    | (Binop (x, bop, _, o, o'), _) :: rest
      when bop = Shl || bop = Lshr || bop = Ashr ->
        ...
        @@ (outstream
           ...
           >@ lift
                Asm.
                  [ (cb bop, [ ~%Rcx; ~%Rax ]); (Movq, [ ~%Rax; co (Loc x) ]) ]
           )
    ...
    | (Binop (x, b, _, o, o'), _) :: rest ->
        ...
        @@ (outstream
           >@ emit_mov (co o) (Reg Rax)
           >@ lift
                Asm.[ (cb b, [ co o'; ~%Rax ]); (Movq, [ ~%Rax; co (Loc x) ]) ]
           )
    | (Alloca (x, at), _) :: rest ->
        loop rest
        @@ (outstream
    ...
    | (Load (x, _, src), _) :: rest ->
        ...
        @@ (outstream
           ...
           >@ lift
                Asm.
                  [ (Movq, [ Ind2 Rax; ~%Rax ]); (Movq, [ ~%Rax; co (Loc x) ]) ]
           )
    ...
    | (Ret (_, None), _) :: rest ->
        ...
        @@ (outstream
           >@ lift
                Asm.[ (Movq, [ ~%Rbp; ~%Rsp ]); (Popq, [ ~%Rbp ]); (Retq, []) ]
           )
    | (Ret (_, Some o), _) :: rest ->
        loop rest
        @@ (outstream
...
let fold_fdecl (f_param : 'a -> uid * Ll.ty -> 'a) (f_lbl : 'a -> lbl -> 'a)
    ...
    (f_term : 'a -> uid * Ll.terminator -> 'a) (init : 'a) (f : Ll.fdecl) : 'a =
  ...
  |> fold_block entry |> fold_lbl_blocks bs
...
let no_reg_layout (f : Ll.fdecl) (_ : liveness) : layout =
  let lo, n_stk =
    fold_fdecl
      ...
      (fun (lo, n) (x, i) ->
        if insn_assigns i then ((x, Alloc.LStk (-(n + 1))) :: lo, n + 1)
...
(* TASK: Implement a (correct) register allocation strategy that
   ...
   registers.ml provides some assign_args_to_graph functions that you can use to
...
(* Added: *)
module UidS = Datastructures.UidS (* Set of Uids *)
module UidM = Datastructures.UidM (* Map of Uids *)
(* *)

(* Types *)
(* Defined above: type layout = { uid_loc : uid -> Alloc.loc; spill_bytes : int } *)
type graph = (UidS.t * Alloc.loc list) UidM.t

(* Added *)
let block_uid_set b =
  List.fold_left
    (fun x (y, z) -> if insn_assigns z then UidS.add y x else x)
    UidS.empty b.insns

(* GRAPH *)

let add_edges graph x =
  List.fold_left
    (fun f (y, z) -> UidM.update (fun (a, b) -> (UidS.add z a, b)) y f)
    graph x

let remove_from_graph e graph =
  UidM.map
    (fun (a, b) -> (UidS.remove (fst e) a, b))
    (UidM.remove (fst e) graph)

(* HELPERS *)

let generate_pairs s =
  if UidS.is_empty s then []
  else
    UidS.fold
      (fun x y ->
        UidS.fold (fun w acc -> (x, w) :: acc) (UidS.remove x s) [] @ y)
      s []

let rec assign_args_to_graph graph list i =
  match arg_reg i with
  | Some c -> (
      match list with
      | [] -> graph
      | x :: xs -> (
          match snd x with
          | Id id ->
              assign_args_to_graph
                (UidM.update (fun (a, b) -> (a, Alloc.LReg c :: b)) id graph)
                xs (i + 1)
          | _ -> assign_args_to_graph graph xs (i + 1)))
  | None -> graph

let add_function_prefix graph (_, i) =
  match i with 
    | Call (_, _, list) -> assign_args_to_graph graph list 0 
    | _ -> graph

let process_block graph b (x : (uid * Alloc.loc) list) (live:liveness) =
  let newgraph =
    List.fold_left
      (fun graph (id, i) ->
        add_function_prefix (add_edges graph (generate_pairs (live.live_in id))) (id, i))
      graph b.insns
  in
  let _, empty = List.partition (fun (_, i) -> insn_assigns i) b.insns in
  let newone =
    List.fold_left (fun xs (id, _) -> (id, Alloc.LVoid) :: xs) x empty
  in
  (newone, add_edges newgraph (generate_pairs (live.live_in (fst b.term))))

let add_argument graph id i =
  match arg_reg i with
  | Some c -> UidM.update (fun (a, b) -> (a, Alloc.LReg c :: b)) id graph
  | None -> graph

let process_arguments graph entry fparam (live : liveness)=
  let id =
    if List.length entry.insns = 0 then fst entry.term
    else fst (List.hd entry.insns)
  in
  let dead = UidS.diff (UidS.of_list fparam) (live.live_in id) in
  let edges =
    UidS.fold
      (fun x y -> UidS.fold (fun z a -> (z, x) :: (x, z) :: a) dead y)
      (live.live_in id) []
  in
  add_edges graph edges

let process_function graph (f : Ll.fdecl) =
  List.fold_left
    (fun (graph, i) id -> (add_argument graph id i, i + 1))
    (graph, 0) f.f_param

let construct_graph (live: liveness) (f : Ll.fdecl) (initialgraph: graph) : (((uid * Alloc.loc) list) * graph) =
  let start, labels = f.f_cfg in
  let x, y =
    List.fold_left
      (fun (x, graph) (l, block) ->
        process_block graph block ((l, Alloc.LLbl (Platform.mangle l)) :: x) live)
      (process_block (process_arguments initialgraph start f.f_param live) start [] live)
      labels
  in
  (x, fst (process_function y f))

let rec simplify_graph (graph: graph) (colors: (UidM.key * (UidS.t * Alloc.loc list)) list) (x : (uid * Alloc.loc) list) spiller : ( ((UidM.key * (UidS.t * Alloc.loc list)) list) * ((uid * Alloc.loc) list)) =
  if UidM.is_empty graph then (colors, x)
  else
    let less = UidM.filter (fun y (a, b) -> UidS.cardinal a < 7) graph in
    if UidM.is_empty less then
      let p = UidM.filter (fun y (a, b) -> List.length b = 0) graph in
      let e = if UidM.is_empty p then UidM.choose graph else UidM.choose p in
      simplify_graph (remove_from_graph e graph) colors ((fst e, spiller ()) :: x) spiller
    else
      let noprefix = UidM.filter (fun y (a, b) -> List.length b = 0) less in
      let e =
        if UidM.is_empty noprefix then UidM.choose less
        else UidM.choose noprefix
      in
      simplify_graph (remove_from_graph e graph) (e :: colors) x spiller


(* Color the nodes in the simplified graph*)
let rec color_graph (colorless: (UidM.key * (UidS.t * Alloc.loc list)) list) (y : (uid * Alloc.loc) list) : (uid * Alloc.loc) list =
  ...
  | x :: xs ->
      ...
      let available_ones =
        LocSet.diff
          LocSet.(
            caller_save |> remove (Alloc.LReg Rax) |> remove (Alloc.LReg Rcx))
          used_ones
      in
      ...
      let prefix =
        List.fold_left
          (fun option location ->
            ...
            | Some u -> Some u
            | None ->
                if LocSet.mem location available_ones then Some location
                else None)
          None
          (snd (snd x))
      in
      ...
      in
      color_graph xs ((fst x, finallocation) :: y)
...
let better_layout (f : Ll.fdecl) (live : liveness) : layout =
  ...
  let nodes : UidS.t =
    let blck, lb_blckl = f.f_cfg in
    List.fold_left
      (fun x (_, block) -> UidS.union (block_uid_set block) x)
      (UidS.union (UidS.of_list f.f_param) (block_uid_set blck))
      lb_blckl
  in

  let initialgraph : graph =
    UidS.fold (fun x y -> UidM.add x (UidS.empty, []) y) nodes UidM.empty
  in
  
  let result : (uid * Alloc.loc) list =
    let (z, graph) = construct_graph live f initialgraph in
    let (colorless, l) = simplify_graph graph [] z spiller in
    color_graph colorless l
  in
  { uid_loc = (fun x -> List.assoc x result); spill_bytes = 8 * !n_spill }
(* *)
...
let check_layout (lay : layout) (live : liveness) (f : Ll.fdecl) =
  ...
  let check_disjoint uid s =
    ...
    if loc <> LVoid then
      UidSet.iter
        (fun v ->
          if v <> uid && loc = lay.uid_loc v then
            failwith
            @@ Printf.sprintf "Invalid layout %s and %s both map to %s" uid v
                 (Alloc.str_loc loc))
        s
  in
  UidSet.iter
